...
public class TicketServiceImpl implements TicketService{
	...
	public int numSeatsAvailable() {
		...
		int tempReservedSeatCount = showTime.getTemporarHeldSeatMap() != null? showTime.getTemporarHeldSeatMap().size() : 0;
		return (venue.getTotalSeats() - (reservedSeatCount + tempReservedSeatCount));
	}
	...
	/**
	 * Method to remove the temporary hold placed over a theater Seat after x number of 
	 * seconds
	 */
	...
	private SeatHold findBestSeats(int seatsForBooking, SeatHold seatHold) {
		...
		for (Map.Entry<String, List<Seat>> entry : reservedSeatMap.entrySet()) {
			...
			int numOfAvailableSeatsForRow = Integer.parseInt(columns) - (numOfSeatsBooked + temporaryHeldSeatList.size());
